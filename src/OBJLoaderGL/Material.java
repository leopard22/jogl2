...
public class Material
{
  ...
  // start rendering using this material's colour information
  {
    if (ka != null) {   // ambient color
      ...
      gl.glMaterialfv(GL.GL_FRONT_AND_BACK, GL2.GL_AMBIENT, colorKa, 0);
    }
    if (kd != null) {  // diffuse color
      float[] colorKd = { kd.getX(), kd.getY(), kd.getZ(), 1.0f };
      gl.glMaterialfv(GL.GL_FRONT_AND_BACK, GL2.GL_DIFFUSE, colorKd, 0);
    }
    if (ks != null) {   // specular color
      float[] colorKs = { ks.getX(), ks.getY(), ks.getZ(), 1.0f };
      gl.glMaterialfv(GL.GL_FRONT_AND_BACK, GL2.GL_SPECULAR, colorKs, 0);
    }

    if (ns != 0.0f) {   // shininess
      gl.glMaterialf(GL.GL_FRONT_AND_BACK, GL2.GL_SHININESS, ns);
    }
